...
( idLoginIndustry INT          NOT NULL
    ...
    , cPwdUUID        UUID         NOT NULL CONSTRAINT DF_Login_cPwdUUID      DEFAULT (gen_random_uuid()) --random uuid to hash password
    , cPwdHash        VARCHAR(100)     NULL
    , cIsFirstLogin   CHAR(1)      NOT NULL CONSTRAINT DF_Login_cIsFirstLogin DEFAULT ('1')
    , dLastChange     DATE             NULL
    , cActive         CHAR(1)      NOT NULL CONSTRAINT  DF_Login_cActive     DEFAULT ('1')
    , CONSTRAINT PK_LoginIndustry       PRIMARY KEY (idLoginIndustry)
    , CONSTRAINT CK_Login_cIsFirstLogin CHECK       (cIsFirstLogin IN ('0','1'))
    , CONSTRAINT CK_Login_cActive       CHECK       (cActive IN ('0','1'))
    , CONSTRAINT CK_Login_dLastChange   CHECK       (dLastChange   <= CURRENT_DATE)
);
